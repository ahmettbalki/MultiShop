...
namespace MultiShop.WebUI.Services.Concrete
{
    public class ClientCredentialTokenService : IClientCredentialTokenService
    {
        ...
        public async Task<string> GetToken()
        {
            ...
            var token1 = await _clientAccessTokenCache.GetAsync("multishoptoken", parameters);
            if (token1 != null)
            {
            ...
            {
                throw new HttpRequestException($"Discovery endpoint error: {discoveryEndPoint.Error}");
            }
            ...
            if (token2.IsError)
            {
                throw new HttpRequestException($"Token request error: {token2.Error}");
            }
